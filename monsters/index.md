# 🗺️ Monster Index

Welcome to the Bestiary.

Each monster represents a real executive function challenge—named, described, and disarmed with humor, science, and ritual. Choose your foe to jump straight to its lore and counter-spells.

<div class="monster-grid">
  <a class="monster-card" href="./slumber-troll.md">
    <div class="monster-card__header">
      <span class="monster-card__emoji">🌙</span>
      <div>
        <p class="monster-card__name">The Slumber Troll</p>
        <p class="monster-card__tagline">Sleep inertia & morning bridges</p>
      </div>
    </div>
    <p class="monster-card__description">Cross from blankets to momentum with wake invocations, light cues, and accountability ropes.</p>
    <div class="monster-card__meta">
      <span class="monster-card__badge">Rituals: Wake Invocation, Beacon of Dawn</span>
      <span class="monster-card__badge">Printable bedside card</span>
    </div>
  </a>

  <a class="monster-card" href="./task-hydra.md">
    <div class="monster-card__header">
      <span class="monster-card__emoji">🐲</span>
      <div>
        <p class="monster-card__name">The Task Hydra</p>
        <p class="monster-card__tagline">Too many heads, not enough focus</p>
      </div>
    </div>
    <p class="monster-card__description">Pick one head to strike, apply the Single-Task Oath, and wield the 3-Item Rule to stop new heads from sprouting.</p>
    <div class="monster-card__meta">
      <span class="monster-card__badge">Rituals: Single-Task Oath, 3-Item Rule</span>
      <span class="monster-card__badge">Printable focus card</span>
    </div>
  </a>

  <a class="monster-card" href="./dopamine-goblin.md">
    <div class="monster-card__header">
      <span class="monster-card__emoji">🧝‍♂️</span>
      <div>
        <p class="monster-card__name">The Dopamine Goblin</p>
        <p class="monster-card__tagline">Shiny distractions & impulse trades</p>
      </div>
    </div>
    <p class="monster-card__description">Contain novelty cravings with reward binding, goblin jars, and clear boundaries around the work you choose.</p>
    <div class="monster-card__meta">
      <span class="monster-card__badge">Rituals: Goblin Jar, Reward Binding</span>
      <span class="monster-card__badge">Tactical environmental tweaks</span>
    </div>
  </a>

  <a class="monster-card" href="./temporal-shark.md">
    <div class="monster-card__header">
      <span class="monster-card__emoji">🦈</span>
      <div>
        <p class="monster-card__name">The Temporal Shark</p>
        <p class="monster-card__tagline">Deadlines you can feel</p>
      </div>
    </div>
    <p class="monster-card__description">Make time visible with Tide Marks, Buddy Buoys, and first bites before the fin is at your feet.</p>
    <div class="monster-card__meta">
      <span class="monster-card__badge">Rituals: Tide Marks, Buddy Buoy</span>
      <span class="monster-card__badge">Printable calendar prompts</span>
    </div>
  </a>

  <a class="monster-card" href="./cave-bear.md">
    <div class="monster-card__header">
      <span class="monster-card__emoji">🐻</span>
      <div>
        <p class="monster-card__name">The Cave Bear</p>
        <p class="monster-card__tagline">Avoidance & hibernation holes</p>
      </div>
    </div>
    <p class="monster-card__description">Step into the light with threshold spells, micro-movements, and social ropes that pull you out of the cave.</p>
    <div class="monster-card__meta">
      <span class="monster-card__badge">Rituals: Threshold, Rope of Returning</span>
      <span class="monster-card__badge">Gentle activation routines</span>
    </div>
  </a>
</div>

More monsters await...

---

🔗 Quick Navigation

<div class="section-callout">
Need a different tool or want to jump back to the guild hall? Choose a shortcut below.
</div>

- 🌙 [The Slumber Troll](slumber-troll.md)
- 🐲 [The Task Hydra](task-hydra.md)
- 🧝‍♂️ [The Dopamine Goblin](dopamine-goblin.md)
- 🦈 [The Temporal Shark](temporal-shark.md)
- 🐻 [The Cave Bear](cave-bear.md)

{% include nav-cards.html %}
